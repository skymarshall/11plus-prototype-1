...
-- RESET: Drop existing tables to allow a clean slate
DROP TABLE IF EXISTS practice_session_questions CASCADE;
DROP TABLE IF EXISTS user_question_attempt_option_order CASCADE;
...
DROP TABLE IF EXISTS question_tag_assignments CASCADE;
...
-- 13. Practice Session Questions
-- Stores the list of questions for a session (persisted at creation)
CREATE TABLE practice_session_questions (
    id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    practice_session_id INT NOT NULL REFERENCES practice_sessions(id) ON DELETE CASCADE,
    question_id INT NOT NULL REFERENCES questions(id) ON DELETE CASCADE,
...
CREATE INDEX idx_practice_session_questions_session ON practice_session_questions(practice_session_id);
...
-- Practice session questions: own rows only (via session)
CREATE POLICY practice_session_questions_select_own ON practice_session_questions
    FOR SELECT USING (
        EXISTS (SELECT 1 FROM practice_sessions ps WHERE ps.id = practice_session_questions.practice_session_id AND ps.user_id = auth.uid())
    );
CREATE POLICY practice_session_questions_insert_own ON practice_session_questions
    FOR INSERT WITH CHECK (
        EXISTS (SELECT 1 FROM practice_sessions ps WHERE ps.id = practice_session_questions.practice_session_id AND ps.user_id = auth.uid())
    );
...
-- =====================================================
-- Backfill Profiles (for existing Auth users after reset)
-- =====================================================
-- Since we dropped the profiles table but auth.users persists, we must recreate profiles for existing users.
INSERT INTO public.profiles (id, display_name)
SELECT 
    id, 
