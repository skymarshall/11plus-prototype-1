...
- **Element:** `explanation` — Character content only. Optional; used for `questions.explanation` after the user answers. **User-facing language only:** do not use the internal term "motif"; use terms such as "symbols" (e.g. "The odd one out contains five symbols; the others contain four").
...
- **Attributes:**
  ...
  - `shading` (optional) — Fill/shading key for the shape: `solid_black`, `grey`, `grey_light`, `white`, `diagonal_slash`, `diagonal_backslash`, `horizontal_lines`, `vertical_lines`. Default per design (e.g. shape outline with optional fill). For shapes that are motifs (see `motif_scale`): default is **`solid_black`** when `shading` is omitted; the generator may set `shading` to override (e.g. `white`, `grey`, `grey_light`).
  ...
  - `motif_scale` (optional) — When present, this shape is drawn as a **motif**: standardized size (small/medium/large per design doc §3.4) and default fill **solid black** unless `shading` is set. **When the template requests motifs** (e.g. "scatter of motifs"), the **generator must set `motif_scale`** on each shape that is a motif (to `small`, `medium`, or `large` as appropriate). If omitted, the renderer uses layout scaling and does not treat the shape as a motif.
...
<question id="nvr-odd-001" template_id="odd-one-out-motif" seed="42">
  ...
  <explanation>The odd one out contains five symbols; the others contain four.</explanation>
  <options>
    <option index="0">
      <diagram>
